...
app.post('/api/animals', requireLogin, requireAdmin, upload.single('imageFile'), (req, res) => {
  ...
  db.run(`INSERT INTO animals (name, breed, age, gender, species, description, image) VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [name, breed, age, gender, species || 'dog', description, imagePath], function(err) {
...
app.put('/api/animals/:id', requireLogin, requireAdmin, upload.single('imageFile'), (req, res) => {
  const { id } = req.params;
  const { name, breed, age, gender, species, description, image } = req.body;
  const imagePath = req.file ? '/uploads/' + req.file.filename : (image || '');
  db.run(`UPDATE animals SET name=?, breed=?, age=?, gender=?, species=?, description=?, image=? WHERE id=?`,
    [name, breed, age, gender, species || 'dog', description, imagePath, id], function(err) {
...
app.delete('/api/admin/applications/:id', requireLogin, requireAdmin, (req, res) => {
  const { id } = req.params;
  db.run(`DELETE FROM applications WHERE id = ?`, [id], function(err) {
    if (err) return res.status(500).json({ error: 'Could not delete application' });
    res.json({ ok: true });
  });
});
